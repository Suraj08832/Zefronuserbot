...
async def convert_to_image(message: Message, client: Client):
    ...
    if message.reply_to_message and message.reply_to_message.sticker:
        ...
        if sticker.is_animated:
            # For animated stickers, we need to download as video
            file_path = await client.download_media(sticker.file_id, file_name="sticker.webm")
        else:
            # For static stickers, download as image
            file_path = await client.download_media(sticker.file_id, file_name="sticker.png")
        return file_path
    return None
